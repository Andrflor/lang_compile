...
Symbol :: struct {
    ...
    defining_scope: ^Symbol,          // The scope that defines this symbol (if it's a scope)
...
Scope_Info :: struct {
    ...
    symbols: map[string]^Symbol,       // Named symbols in this scope
    anonymous: [dynamic]^Symbol,             // Anonymous symbols
...
init_scope :: proc(parent: ^Scope_Info) -> ^Scope_Info {
    ...
    scope.anonymous = make([dynamic]^Symbol, 0, 8)
 ...
 */
create_symbol :: proc(name: string, node: ^Node, scope: ^Scope_Info, kind: Reference_Kind, position: Position) -> ^Symbol {
    symbol := new(Symbol)
    symbol.name = name
    symbol.node = node
    symbol.scope = scope
...
add_symbol :: proc(analyzer: ^Analyzer, symbol: ^Symbol) -> ^Symbol {
    if symbol.name != "" {
        ...
        if existing, ok := map.get(&analyzer.current_scope.symbols, symbol.name); ok {
            if symbol.kind == .Definition || symbol.kind == .PullDefinition {
                add_error(analyzer, fmt.tprintf("Symbol '%s' is already defined in this scope", symbol.name), symbol.position)
                return existing
            }
        }

        map.set(&analyzer.current_scope.symbols, symbol.name, symbol)
    } else {
        append(&analyzer.current_scope.anonymous, symbol)
    }
 ...
 */
add_reference :: proc(analyzer: ^Analyzer, symbol: ^Symbol, node: ^Node, kind: Reference_Kind, position: Position) -> ^Reference {
...
add_error :: proc(analyzer: ^Analyzer, message: string, position: Position) {
    error_message := fmt.tprintf("Error at line %d, column %d: %s",
...
add_warning :: proc(analyzer: ^Analyzer, message: string, position: Position) {
    warning_message := fmt.tprintf("Warning at line %d, column %d: %s",
                                  position.line, position.column, message)
    append(&analyzer.warnings, warning_message)
    fmt.eprintln(warning_message)
}
 ...
 */
create_scope_for_node :: proc(analyzer: ^Analyzer, node: ^Node, name: string, position: Position) -> ^Scope_Info {
    // Create scope
    ...
    symbol.defining_scope = symbol  // Self-reference for scopes

    // Register scope
    scope.scope_symbol = symbol
    symbol.defining_scope = symbol
...
lookup_symbol :: proc(analyzer: ^Analyzer, name: string) -> ^Symbol {
    ...
    for current != nil {
        ...
        if symbol, ok := map.get(&current.symbols, name); ok {
            return symbol
        }
...
lookup_symbol_in_scope :: proc(scope: ^Scope_Info, name: string) -> ^Symbol {
    if symbol, ok := map.get(&scope.symbols, name); ok {
        return symbol
    }
...
lookup_symbol_local :: proc(analyzer: ^Analyzer, name: string) -> ^Symbol {
    if symbol, ok := map.get(&analyzer.current_scope.symbols, name); ok {
        return symbol
    }
 ...
 */
analyze_pointing :: proc(analyzer: ^Analyzer, node: ^Node, pointing: Pointing, position: Position) {
 ...
 */
analyze_pointing_pull :: proc(analyzer: ^Analyzer, node: ^Node, pointing_pull: PointingPull, position: Position) {
 ...
 */
analyze_event_push :: proc(analyzer: ^Analyzer, node: ^Node, event_push: EventPush, position: Position) {
    ...
    if id, ok := event_push.name^.(Identifier); ok {
        ...
        if symbol != nil {
            ...
            add_error(analyzer, fmt.tprintf("Undefined event target '%s'", id.name), position)
        }
    }
 ...
 */
analyze_event_pull :: proc(analyzer: ^Analyzer, node: ^Node, event_pull: EventPull, position: Position) {
 ...
 */
analyze_resonance_push :: proc(analyzer: ^Analyzer, node: ^Node, resonance_push: ResonancePush, position: Position) {
 ...
 */
analyze_resonance_pull :: proc(analyzer: ^Analyzer, node: ^Node, resonance_pull: ResonancePull, position: Position) {
 ...
 */
analyze_override :: proc(analyzer: ^Analyzer, node: ^Node, override: Override, position: Position) {
    // Analyze the base
...
analyze_pattern :: proc(analyzer: ^Analyzer, node: ^Node, pattern: Pattern, position: Position) {
    ...
    for i := 0; i < len(pattern.value); i += 1 {
        ...
        branch_info.captured_symbols = map.make(map.Map(string, ^Symbol))
 ...
 */
analyze_constraint :: proc(analyzer: ^Analyzer, node: ^Node, constraint: Constraint, position: Position) {
    ...
    // If constraint is an identifier, check if it exists
    if id, ok := constraint.constraint^.(Identifier); ok {
        constraint_symbol := lookup_symbol(analyzer, id.name)
        if constraint_symbol == nil {
            add_error(analyzer, fmt.tprintf("Undefined constraint '%s'", id.name), position)
        } else {
            constraint_info.constraint = constraint_symbol
            add_reference(analyzer, constraint_symbol, node, .Constraint, position)
        }
    }
...
analyze_expand :: proc(analyzer: ^Analyzer, node: ^Node, expand: Expand, position: Position) {
    ...
    if id, ok := expand.target^.(Identifier); ok {
        ...
        if symbol == nil {
            ...
            if symbol.defining_scope != nil && symbol.defining_scope.value != nil {
                append(&analyzer.current_scope.expansions, symbol.defining_scope.value)
            }
        }
    }
}
...
validate_definitions_in_scope :: proc(analyzer: ^Analyzer, scope: ^Scope_Info) {
    ...
    for _, symbol in scope.symbols {
        ...
        if symbol.defining_scope != nil && symbol.defining_scope.value != nil {
            validate_definitions_in_scope(analyzer, symbol.defining_scope.value)
        }
    }

    // Check anonymous symbols too
    for symbol in scope.anonymous {
        for reference in symbol.references {
            validate_reference(analyzer, reference)
        }
...
validate_reference :: proc(analyzer: ^Analyzer, reference: ^Reference) {
    ...
    case .Constraint:
        // TODO: Check that the symbol can be used as a constraint

    case .Override:
        // TODO: Check that the symbol can be overridden

    case .EventPush, .EventPull:
        // TODO: Check event handlers

    case .ResonancePush, .ResonancePull:
        // TODO: Check resonance bindings
    }
}
...
validate_constraints_in_scope :: proc(analyzer: ^Analyzer, scope: ^Scope_Info) {
    ...
    for constraint in scope.constraints {
        if constraint.constraint != nil {
            // TODO: Check if the constrained value matches the constraint
        }
    }
    ...
    for _, symbol in scope.symbols {
        if symbol.defining_scope != nil && symbol.defining_scope.value != nil {
            validate_constraints_in_scope(analyzer, symbol.defining_scope.value)
        }
    }
...
validate_pattern_matches_in_scope :: proc(analyzer: ^Analyzer, scope: ^Scope_Info) {
    ...
    for _, symbol in scope.symbols {
        if symbol.defining_scope != nil && symbol.defining_scope.value != nil {
            validate_pattern_matches_in_scope(analyzer, symbol.defining_scope.value)
        }
    }
...
validate_resonance_bindings_in_scope :: proc(analyzer: ^Analyzer, scope: ^Scope_Info) {
    ...
    for resonance in scope.resonances {
        if resonance.target != nil {
            if resonance.is_push {
                ...
                // Count how many times this symbol is driven
                drive_count := 0
                for r in scope.resonances {
                    if r.is_push && r.target == resonance.target {
                        drive_count += 1
                    }
                }
                ...
                if drive_count > 1 {
                    ...
                    add_error(analyzer, fmt.tprintf("Symbol '%s' is driven by multiple resonances",
                                                  resonance.target.name), position)
                }
            }
        }
    }
    ...
    for _, symbol in scope.symbols {
        if symbol.defining_scope != nil && symbol.defining_scope.value != nil {
            validate_resonance_bindings_in_scope(analyzer, symbol.defining_scope.value)
        }
    }
...
print_scope :: proc(scope: ^Scope_Info, indent: int) {
    ...
    for name, symbol in scope.symbols {
        print_symbol(symbol, indent + 2)
    }

    // Print anonymous symbols
    for symbol in scope.anonymous {
        print_symbol(symbol, indent + 2)
    }
 ...
 */
print_symbol :: proc(symbol: ^Symbol, indent: int) {
    if symbol == nil {
        return
    }
    ...
    if symbol.name != "" {
        fmt.printf("%s%s", indent_str, symbol.name)
    } else {
        fmt.printf("%s<anonymous>", indent_str)
    }
    ...
    if symbol.defining_scope != nil && symbol.defining_scope.value != nil {
        print_scope(symbol.defining_scope.value, indent + 2)
    }
}
...
is_reachable :: proc(from_scope: ^Scope_Info, symbol: ^Symbol) -> bool {
    ...
    for current != nil {
        ...
        for name, s in current.symbols {
            if s == symbol {
                return true
            }
        }

        // Check anonymous symbols
        for s in current.anonymous {
            if s == symbol {
                return true
            }
        }
