...
init_analyzer :: proc(ast: ^Node, filename: string) -> ^Analyzer {
...
analyze_ast :: proc(ast: ^Node, filename: string) -> ^Analyzer {
	// Initialize analyzer with the AST root as the global scope
	analyzer := init_analyzer(ast, filename)
	...
	if id, is_id := pointing.name^.(Identifier); is_id {
		...
		if type_id, is_type_id := constraint.constraint^.(Identifier); is_type_id {
			...
			if constraint_symbol == nil {
				add_error(
					analyzer,
					fmt.tprintf("Undefined constraint type '%s'", type_id.name),
					position,
				)
			}
		} else {
...
validate_constraint :: proc(analyzer: ^Analyzer, constraint: Constraint_Info) {
	...
	if target.constraint != nil && target.constraint != constraint.constraint {
		...
		add_error(
			analyzer,
			fmt.tprintf(
				"Incompatible constraints for '%s': %s and %s",
				target.name,
			...
			if event.is_push && event.event != nil {
				if !event_handlers[event.event.id] {
					add_warning(
						analyzer,
						fmt.tprintf(
							"Event '%s' is pushed but not handled by any event handler",
							event.event.name,
						),
...
print_scope :: proc(scope: ^Scope_Info, depth: int) {
	...
	if scope.defining_symbol != nil {
		...
		fmt.printf(
			"%sScope: %s (id:%d, symbol_id:%d, node_type:%s)\n",
			indent,
			scope_name,
			scope.id,
			scope.defining_symbol.id,
			node_type_name,
		)
	} else {
		node_type_name := "nil"
		...
		fmt.printf(
			"%sScope: <unnamed> (id:%d, no_defining_symbol, node_type:%s)\n",
			indent,
			scope.id,
			node_type_name,
		)
	...
	for symbol in scope.symbol_list {
		...
		if symbol.name != "" {
			fmt.printf(
				"%s  Symbol: %s (id:%d, index:%d, node_type:%s, kind:%v%s)\n",
				indent,
				symbol.name,
				symbol.id,
				symbol.index,
			...
			fmt.printf(
				"%s  Symbol: <anonymous> (id:%d, index:%d, node_type:%s, kind:%v%s)\n",
				indent,
				symbol.id,
				symbol.index,
				node_type,
